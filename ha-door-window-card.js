  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      *{box-sizing:border-box}
      ...
      .summary .stat{padding:12px 14px;border:1px solid var(--edge);border-radius:16px;text-align:center}
      ...
      .tile{position:relative;padding:14px;border:1px solid var(--edge);border-radius:18px;cursor:pointer;overflow:hidden;background:color-mix(in srgb,var(--primary-text-color) 2%,transparent);transition:border-color .25s ease,background .25s ease}
      .tile.open{border-color:color-mix(in srgb,var(--warn) 45%,var(--edge));background:color-mix(in srgb,var(--warn) 7%,transparent)}
      .tile.long-open{animation:long-open-pulse 2.4s ease-in-out infinite}
      @keyframes long-open-pulse{0%,100%{box-shadow:0 0 0 0 color-mix(in srgb,var(--warn) 0%,transparent)}50%{box-shadow:0 0 0 4px color-mix(in srgb,var(--warn) 20%,transparent)}}
